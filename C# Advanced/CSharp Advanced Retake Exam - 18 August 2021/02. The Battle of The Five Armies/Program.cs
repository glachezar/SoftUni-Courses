...
namespace _02._The_Battle_of_The_Five_Armies
{
    class Army
    {
        ...
        public void Move(string command, char[][] world)
        {
            ...
            switch (command)
            {
                ...
                case "down": if (this.Row + 1 < world.Length) this.Row++; break;

                case "left": if (this.Col - 1 >= 0) this.Col--; break;

                case "right": if (this.Col + 1 < world[0].Length) this.Col++; break;
            }
            ...
            if (this.Armor <= 0) world[this.Row][this.Col] = 'X';
        }
    }
    ...
    internal class Program
    {
        ...
        {
            if (army.HasWon) Console.WriteLine($"The army managed to free the Middle World! Armor left: {army.Armor}");

            else Console.WriteLine($"The army was defeated at {army.Row};{army.Col}.");

            for (int row = 0; row < world.Length; row++) Console.WriteLine(String.Join("", world[row]));
        }
    }
